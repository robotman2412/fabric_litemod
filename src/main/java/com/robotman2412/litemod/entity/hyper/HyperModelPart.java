...
public class HyperModelPart extends ModelPart {
	...
	public HyperModelPart(Model model) {
		super(model);
	...
	public HyperModelPart(Model model, int textureOffsetU, int textureOffsetV) {
		this(model.textureWidth, model.textureHeight, textureOffsetU, textureOffsetV);
		model.accept(this);
	}
	
	public HyperModelPart(int textureWidth, int textureHeight, int textureOffsetU, int textureOffsetV) {
	...
	public void addCuboid(int u, int v, float x, float y, float z, float sizeX, float sizeY, float sizeZ, float extraX, float extraY, float extraZ, boolean mirror, boolean bl) {
		this.cuboids.add(new Cuboid(u, v, x, y, z, sizeX, sizeY, sizeZ, extraX, extraY, extraZ, mirror, this.textureWidth, this.textureHeight));
	}
	
	public void cube(float x, float y, float z, float sizeX, float sizeY, float sizeZ, boolean mirror, HyperCuboidUVMapping uv) {
		this.cuboids.add(new Cuboid(x, y, z, sizeX, sizeY, sizeZ, mirror, textureWidth, textureHeight, uv));
	}
	...
	private void renderCuboids(MatrixStack.Entry matrices, VertexConsumer vertexConsumer, int light, int overlay, float red, float green, float blue, float alpha) {
		...
		while(var11.hasNext()) {
			Cuboid cuboid = (Cuboid)var11.next();
			Quad[] var13 = cuboid.sides;
			int var14 = var13.length;
			
			for(int var15 = 0; var15 < var14; ++var15) {
				Quad quad = var13[var15];
	...
	public HyperModelPart setTextureSize(int width, int height) {
	...
	public static class Cuboid {
		...
		public Cuboid(float x, float y, float z, float sizeX, float sizeY, float sizeZ, boolean mirror,
					  float textureWidth, float textureHeight, HyperCuboidUVMapping uv) {
			this.minX = x;
			this.minY = y;
			this.minZ = z;
			this.maxX = x + sizeX;
			...
			HyperUVMapping side = uv.bottom;
			this.sides[2] = new Quad(new Vertex[]{vertex6, vertex5, vertex, vertex2},
					side.u0, side.v0, side.u1, side.v1, textureWidth, textureHeight, mirror, Direction.DOWN); //bottom
			side = uv.top;
			this.sides[3] = new Quad(new Vertex[]{vertex3, vertex4, vertex8, vertex7},
					side.u0, side.v0, side.u1, side.v1, textureWidth, textureHeight, mirror, Direction.UP); //top
			side = uv.right;
			this.sides[1] = new Quad(new Vertex[]{vertex, vertex5, vertex8, vertex4},
					side.u0, side.v0, side.u1, side.v1, textureWidth, textureHeight, mirror, Direction.WEST); //right
			side = uv.backward;
			this.sides[4] = new Quad(new Vertex[]{vertex2, vertex, vertex4, vertex3},
					side.u0, side.v0, side.u1, side.v1, textureWidth, textureHeight, mirror, Direction.NORTH); //backward
			side = uv.left;
			this.sides[0] = new Quad(new Vertex[]{vertex6, vertex2, vertex3, vertex7},
					side.u0, side.v0, side.u1, side.v1, textureWidth, textureHeight, mirror, Direction.EAST); //left
			side = uv.forward;
			this.sides[5] = new Quad(new Vertex[]{vertex5, vertex6, vertex7, vertex8},
					side.u0, side.v0, side.u1, side.v1, textureWidth, textureHeight, mirror, Direction.SOUTH); //forward
		}
